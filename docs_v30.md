...
- **Hide** — the group actively conceals itself, increasing how difficult it is for other factions to detect them. Because characters and groups are living objects, their `visibility` and `ceiling visibility` are runtime values (see Section 4.3.1). Each completed Hide execution raises the group's `ceiling visibility`, pushing the Known and Located thresholds higher and forcing other factions to accumulate more visibility to maintain or achieve the same awareness state. A faction that had Located the hiding group may lose that state and revert to Known or Undiscovered if the group's `ceiling visibility` rises above the faction's current visibility relative to the new thresholds. A sufficiently capable opposing scout or sustained search effort can still locate a hiding group by continuing to accumulate faction visibility past the new higher thresholds.
