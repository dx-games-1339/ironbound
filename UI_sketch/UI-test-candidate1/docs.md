...
**Navigation:** Clicking a city on the Global Map opens a City Details Popup — identical in structure to the POI Details Popup. The panel displays the city's name, size, and a "never" despawn value (cities never despawn). An **Enter City** button navigates into the City Layout. Cities are always in the Discovered state and the Enter City button is always enabled.
...
**POI metadata** shown in the POI Details Popup:
- POI type and name
...
**Terrain types and canonical colours:**

Each terrain type has a defined canonical display colour. These colours are fixed design decisions and must be used consistently across the map renderer, the terrain colour legend, and any documentation visuals.
...
Generation rules ensure every terrain type occupies at least 5% of map tiles. Terrain distribution is driven by three noise-based axes — elevation, moisture, and temperature — with a post-generation redistribution pass to enforce the minimum coverage guarantee.
...
This section will document the full rules governing procedural world generation, including terrain noise parameters, city placement constraints, POI spawn logic, and the redistribution pass that guarantees minimum terrain coverage. It will be written after the UI specification is complete.
